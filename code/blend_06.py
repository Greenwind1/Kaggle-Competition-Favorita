...
csv_l = np.sort(glob('./sub/blend/ensemble_cnn_02-5.csv')).tolist()
csv_l += np.sort(glob('./sub/blend/ensemble_cnn_04-5.csv')).tolist()
...
pred_b.to_csv(f'./sub/blend/blend_06-{len(pred_l)}_{tuple(w_l)}.csv',
              index=False)
