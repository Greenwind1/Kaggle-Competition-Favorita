...
print(
    ...
    '\nNunique of item in the intersection of train and test =',
    len(item_inter)
)
...
#   Validation
# -------------------------------------------------------------------
w_l = np.sort(glob(MODEL_DIR + '/weights_*.h5'))
val_ensemble = 0
test_ensemble = 0
...
for w in w_l:
    ...
    val_ensemble += val_pred / len(w_l)
    test_pred = model.predict(Xtest)
    test_ensemble += test_pred / len(w_l)
    score_l = cal_score(Yval, val_pred)
    print(
        f'Day all, 0-5, 6-16 = {score_l[0]}, {score_l[1]}, {score_l[2]}'
    )

score_ensemble = cal_score(Yval, val_ensemble)
print(
    f'Day all, 0-5, 6-16 = {score_ensemble}'
)
...
# -------------------------------------------------------------------
make_submission(
    df_index, test_ensemble,
    f'./sub/dnn/cnn_epoch-ensemble_{NAME}_epoch{EPOCH}_{score_ensemble[0]}.csv'
)
gc.collect()
