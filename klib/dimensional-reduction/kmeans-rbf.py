...
all_data = hstack((scale(
    vstack((train_num, test_num)).astype(np.float64)).astype(np.float32),
...
for n_clusters in [25, 50, 75, 100, 200]:
    ...
    Dataset.save_part_features(part_name,
                               ['cluster_rbf_%d_%d' % (n_clusters, i) for i in
                                xrange(n_clusters)])
