...
'''
8-Fold training with 3 averaged runs per fold.
Results may improve with more folds & runs.
'''
...
def build_embedding_network():
    ...
    model_ps_ind_02_cat.add(Embedding(5, 3, input_length=1))  # 5 => 3
...
# network training
# -------------------------------------------------------------------------------
K = 8  # more fold, more data ==> NN will work.
runs_per_fold = 10
n_epochs = 20
...
for i, (f_ind, outf_ind) in enumerate(kfold.split(X_train, y_train)):
    ...
    for j in range(runs_per_fold):
        ...
        NN.fit(proc_X_train_f, y_train_f.values,
               nb_epoch=n_epochs, batch_size=4096, verbose=1)
...
# K = 8   runs_per_fold = 3   n_epochs = 15   CV: .27834
# K = 8   runs_per_fold = 5   n_epochs = 15   CV: .27838
# K = 8   runs_per_fold = 5   n_epochs = 20   CV: .27967
# K = 8   runs_per_fold = 10   n_epochs = 20   CV: .27967
# K = 8   runs_per_fold = 5   n_epochs = 30   CV: .27554
# K = 10   runs_per_fold = 3   n_epochs = 15   CV: .27757
...
df_sub = pd.DataFrame({'id': df_test.id,
                       'target': y_pred_final},
                      columns=['id', 'target'])
df_sub.to_csv('./submission/DNN/NN_EntityEmbed_10fold-sub.csv', index=False)

pd.DataFrame(full_val_preds).to_csv('./stacking_data/NN_EntityEmbed_10fold-val_preds.csv',
                                    index=False)
